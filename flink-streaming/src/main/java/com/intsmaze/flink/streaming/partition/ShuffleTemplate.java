...
public class ShuffleTemplate {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String flag = " shuffle分区策略前子任务名称:";
        ...
        DataStream<Trade> mapTwo = mapOne.shuffle();

        DataStream<Trade> mapThree = mapTwo.map(new RichMapFunction<Trade, Trade>() {
            @Override
            public Trade map(Trade value) {
                System.out.println("元素值:" + value + " shuffle分区策略后子任务名称:" + getRuntimeContext().getTaskNameWithSubtasks()
                        + " ,子任务编号:" + getRuntimeContext().getIndexOfThisSubtask());
                return value;
            }
        });
